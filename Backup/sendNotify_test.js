...
// 3.添加BARK_ICON参数读取
// 4.修复捕获账号CK失效后，sendNotify正在处理账号Remark时，青龙面板 API（获取环境变量备注）访问失败导致HTTPError: Response code 401的问题。不再调用 getEnvs() 读取备注，改为直接读取本地 JSON 文件
...
async function sendNotify(text, desp, params = {}, author = "",strsummary="") {
    ...
    try {
        ...
        if (ShowRemarkType != "1" && titleIndex3 == -1) {
            ...
            const remarkFile = "/ql/data/scripts/CKName_cache.json";
            let localRemarks = [];
            try {
                if (fs.existsSync(remarkFile)) {
                    localRemarks = JSON.parse(fs.readFileSync(remarkFile, "utf8"));
                    console.log(`已读取本地备注文件`);
                } else {
                    console.log(`未找到本地备注文件：${remarkFile}`);
                }
            } catch (e) {
                console.log("读取或解析 CKName_cache.json 失败:", e);
                localRemarks = [];
            }
            ...
            if (localRemarks.length > 0) {
                ...
                for (let i = 0; i < localRemarks.length; i++) {
                    const pt_pin = localRemarks[i].pt_pin;
                    const remark = localRemarks[i].nickName || "";
                    if (!pt_pin) continue;

                    // nickName 优先使用 remark
                    $.UserName = pt_pin;
                    $.nickName = remark || pt_pin;
                    $.Remark = remark;
                    ...
                    try {
                        ...
                        console.log("替换出错了: " + err);
                    }
                }

                console.log("Remark处理完成，开始发送通知...");
                if (strAllNotify) {
                    desp = strAllNotify + "\n" + desp;
                }
            }
...
async function isLoginByX1a0He(cookie) {
    console.log(`正在调用isLoginByX1a0He(cookie)`);
...
async function getEnvByPtPin(Ptpin){
    console.log('正在调用getEnvByPtPin(Ptpin)');
  const envs = await getEnvs();
  for (let i = 0; i < envs.length; i++) {	
