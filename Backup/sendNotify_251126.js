...
// 3.添加BARK_ICON参数读取。
// 4.修复sendNotify正在处理账号Remark时，青龙面板 API 访问失败返回401的问题。不再调用 getEnvs() 读取备注，改为直接读取本地 JSON 文件。
// 5.修复捕获账号CK失效后，自动禁用失效CK时，青龙面板 API 访问失败返回401的问题。添加401容错机制，跳过执行自动禁用失效CK，仅发送通知。
        ...
        if (process.env.NOTIFY_AUTOCHECKCK == "true") {
            if (text.indexOf("已失效") != -1 || desp.indexOf("登录") != -1) {
                ...
                if (strPtPin) {
                    ...
                    if (!temptest) {
                        console.log(`未找到` + strPtPin + `对应的环境变量，跳过自动禁用\n`);
                        text = "京东CK检测";
